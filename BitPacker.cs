...
namespace Bitpacking
{
    ...
    {
        #region Constants
        private const int _DEFAULT_BUFFER_SIZE = 300;
        private const int _BUFFER_BIT_SIZE = 32;     // Number of bits of an element of _buffer (List<uint> => 32 bits)
        #endregion Constants
        ...
        public ulong BitLength { get { return (ulong)BitIndex + (ulong)(WordIndex * _BUFFER_BIT_SIZE); } }
        ...
        {
            _buffer = new uint[(int)Math.Ceiling(pByteBufferSize / (decimal)4)];
            _temp = 0;
            _offsetBitReaded = 0;
            BitIndex = 0;
            WordIndex = 0;
        ...
        public BitPacker(Span<uint> pBuffer, int? pBitLength = null)
        {
            ...
            if (pBitLength.HasValue)
                ...
                bitLength = pBuffer.Length * _BUFFER_BIT_SIZE;
            _temp = 0;
            _offsetBitReaded = 0;
            BitIndex = bitLength % _BUFFER_BIT_SIZE;
            WordIndex = bitLength / _BUFFER_BIT_SIZE;
        }
        ...
        public static BitPacker FromArray(byte[] pBuffer, bool pPushTempInBuffer = true)
        {
            ...
            int nbOfBytePerIndex = _BUFFER_BIT_SIZE / 8;
        ...
        public void WriteValue(uint pValue, int pNbOfBits)
        {
            ...
            if (_buffer == null)
                _buffer = new uint[_DEFAULT_BUFFER_SIZE];
            ulong maskNbOfBits = (1ul << pNbOfBits) - 1;
            _temp |= (pValue & maskNbOfBits) << BitIndex;
            BitIndex += pNbOfBits;
            while (BitIndex >= _BUFFER_BIT_SIZE)
            {
                PushTempInBuffer();
            }
        }
        ...
        public void OverrideValue(uint pValue, int pNbOfBits, int pStartPosition = 0)
        {
            ...
            int bufferWordIndex = pStartPosition / _BUFFER_BIT_SIZE;
            int start = pStartPosition % _BUFFER_BIT_SIZE;
            ...
            if (_BUFFER_BIT_SIZE < start + pNbOfBits && bufferWordIndex + 1 <= WordIndex)
            {
                int s = start - _BUFFER_BIT_SIZE + pNbOfBits;
        ...
        public void InsertBytes(byte[] pData, int pLength)
        {
            ...
            WordIndex += bitsAdded / _BUFFER_BIT_SIZE;
            BitIndex = bitsAdded % _BUFFER_BIT_SIZE;
        }
        ...
        public void PushTempInBuffer()
        {
            if (BitIndex > 0)
            {
                ...
                if (BitIndex >= _BUFFER_BIT_SIZE)
                {
                    _temp >>= _BUFFER_BIT_SIZE;
                    BitIndex -= _BUFFER_BIT_SIZE;
                    WordIndex++;
                }
            }
        }
        ...
        public uint ReadValue(int pNbOfBits, bool pRemoveBits = true, int pStartPosition = 0)
        {
            ...
            int bufferWordIndex = pStartPosition / _BUFFER_BIT_SIZE;
            int start = (pStartPosition + _offsetBitReaded) % _BUFFER_BIT_SIZE;
            Span<uint> spanBuffer = GetUintSpanBuffer();
            if ((bufferWordIndex + pNbOfBits / _BUFFER_BIT_SIZE) <= spanBuffer.Length)
            {
                if (pNbOfBits <= _BUFFER_BIT_SIZE)
                {
                    uint startingVal = 0;
                    if (start > 0)
                    {
                    ...
                    // If the value to be written overlaps on two registers
                    if (_BUFFER_BIT_SIZE < start + pNbOfBits && bufferWordIndex + 1 < spanBuffer.Length)
                    {
                        int s = start - _BUFFER_BIT_SIZE + pNbOfBits;
                    ...
                    if (pRemoveBits)
                    {
                        if (pStartPosition > 0)
                        {
                            ...
                            for (int i = spanBuffer.Length - 1; i >= bufferWordIndex; i--)
                            {
                                ...
                                spanBuffer[i] |= outVal << (_BUFFER_BIT_SIZE - pNbOfBits);
                                outVal = temp;
                            }
                        }
                        else
                ...
                {
                    throw new Exception("Number of bits to read exceed the maximum size of " + _BUFFER_BIT_SIZE + " bits.");
                }
            }
            else
            {
        ...
        public byte[] ReadBytes(int pLength)
        {
            ...
            _offsetBitReaded += bitsToRemove;
            WordIndex -= bitsToRemove / _BUFFER_BIT_SIZE;
            BitIndex -= bitsToRemove % _BUFFER_BIT_SIZE;
            return result;
        }
        ...
        public void RemoveFromEnd(int pNbOfBits)
        {
            if (pNbOfBits > 0)
            {
                int wordToRemove = pNbOfBits / _BUFFER_BIT_SIZE;
                int bitsToRemove = pNbOfBits % _BUFFER_BIT_SIZE;
                Span<uint> data = new Span<uint>(_buffer, WordIndex - wordToRemove, wordToRemove + (int)Math.Ceiling((decimal)bitsToRemove / _BUFFER_BIT_SIZE));
        ...
        public Span<byte> GetByteSpanBuffer(int? pLength = null)
        {
            ...
            return MemoryMarshal.Cast<uint, byte>(GetUintSpanBuffer()).Slice((int)Math.Ceiling((decimal)(_offsetBitReaded % _BUFFER_BIT_SIZE) / 8), size);
        }

        public Span<uint> GetUintSpanBuffer()
        {
            int wordStart = _offsetBitReaded / _BUFFER_BIT_SIZE;
            byte bitStart = (byte)(_offsetBitReaded % _BUFFER_BIT_SIZE);
            return new Span<uint>(_buffer, wordStart, (int)Math.Ceiling((decimal)(BitLength + bitStart) / _BUFFER_BIT_SIZE));
        }
